...
function toStr(bm: BurrowMove): string {
    const key: string = bm.burrow.map(a => {
        const [x, y] = a.coord;
        return `id:${a.id}seq:${a.seq}x:${x}y:${y}s:${a.state}`;
    }).join();
    return key;
}
...
function isDest(b: Burrow): boolean {
    return b.every((a, _, all) => {
        ...
        return (x === ex2 && y === ey2) ||
            (x === ex1 && y === ey1 && all.find(v => v.id === a.id && v.seq !== a.seq && v.coord[0] === ex2 && v.coord[1] === ey2));
    });
}
    ...
    if (!mustMoveDone) {
        for (let i = 0; i < bm.burrow.length; i++) {
            ...
            if (amphipod.state === AmphipodState.None) {
                ...
                if (inExit1 === undefined && inExit2 === undefined && isPathFree(bm.burrow, emptyMap, x, exit1X)) {
                    moves.push({
                        burrow: cloneBurrowWithAmphipod(bm.burrow, { id: amphipod.id, seq: amphipod.seq, coord: [x, y], state: AmphipodState.Entering }),
                        energy: 0
                    });
                } else if (inExit1 === undefined && inExit2 !== undefined && inExit2.id === amphipod.id && isPathFree(bm.burrow, emptyMap, x, exit1X)) {
                    moves.push({
                        burrow: cloneBurrowWithAmphipod(bm.burrow, { id: amphipod.id, seq: amphipod.seq, coord: [x, y], state: AmphipodState.Entering }),
                        energy: 0
                    });
