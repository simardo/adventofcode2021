...
function doPart(input: string): string | number {
    const tileOrigin: number[][] = input.split('\n').map(s => [...s].map(n => Number.parseInt(n)));

    for (let y: number = 0; y < tileOrigin.length; y++) {
        const line: number[] = tileOrigin[y];
        const xLength: number = line.length;
        for (let x: number = xLength + 1; x <= xLength * 5; x++) {
            let v: number = line[x - xLength - 1] + 1;
            if (v > 9) {
                v = 1;
            }
            line.push(v);
    ...
    const yLength: number = tileOrigin.length;
    for (let y: number = 0; tileOrigin.length < yLength * 5; y++) {
        const line: number[] = tileOrigin[y];
        const newLine: number[] = line.map(l => {
            let v: number = l + 1;
            if (v > 9) {
                v = 1;
            }
        ...
        tileOrigin.push(newLine);
    }

    const map: number[][] = tileOrigin.map(line => line.map(v => v));
